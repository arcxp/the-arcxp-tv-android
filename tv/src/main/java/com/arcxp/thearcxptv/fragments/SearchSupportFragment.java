...
public class SearchSupportFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        searchFrame.setOnFocusSearchListener(new BrowseFrameLayout.OnFocusSearchListener() {
            ...
            public View onFocusSearch(View focused, int direction) {
                //If there are no sarch results then automatically focus on the search bar
                if (mRowsSupportFragment != null && mRowsSupportFragment.getView() != null
                        && mRowsSupportFragment.getView().hasFocus()) {
                            return mSearchBar;
                } else if (mSearchBar.hasFocus() && direction == FOCUS_DOWN) {
        ...
        // Make sure that navigation when there are no search results focuses on the
        // search bar rather than the empty results frame
        searchFrame.setOnChildFocusListener(new BrowseFrameLayout.OnChildFocusListener() {
            ...
            public void onRequestChildFocus(View child, View focused) {
                if (focused instanceof VerticalGridView &&
                        ((mRowsSupportFragment == null || mRowsSupportFragment.getView() == null) ||
                                mResultAdapter == null || mResultAdapter.size() <= 0))
    ...
    void updateSearchBarVisibility() {
        ...
        mSearchBar.setVisibility(position <=0 || mResultAdapter == null
                || mResultAdapter.size() == 0 ? VISIBLE : GONE);
    }
